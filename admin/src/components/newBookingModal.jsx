...
import { newBookingDetails, uploadImageToCloudinary } from "@/apiClient";
...
export default function NewBookingModal({
      ...
      try {
        const promise = () =>
          new Promise(async (resolve, reject) => {
            try {
              const response = await newBookingMutation.mutateAsync({
                data: { ...values, isOnline: false },
              });
              resolve(response);
            } catch (error) {
              reject(error);
  ...
  useEffect(() => {
    if (checkIn && checkOut) {
      formik.setValues({
        ...initialValues,
        checkIn: new IndianDate(checkIn),
        checkOut: new IndianDate(checkOut),
      });
    }
  }, [checkIn, checkOut]);
  ...
  const newBookingMutation = useMutation(newBookingDetails);
  ...
  const handleFormSubmit = async () => {
    ...
    if (Object.keys(errors).length === 0) {
      ...
      const arrayOfErrorKeyNames = await Object.keys(errors).filter(
        (err) => typeof errors[err] === "string"
      );
      const firstErrorKey = await errors.guestInfo.map((obj) =>
        !obj ? {} : obj
      );
      let arrayOfNames = await firstErrorKey?.flatMap((obj, index) => {
        return Object?.keys(obj).map((key) => `guestInfo.${index}.${key}`);
      });
      arrayOfNames = [...arrayOfErrorKeyNames, ...arrayOfNames];
        ...
        >
          <div className="items-end gap-6 grid grid-cols-2 px-6">
            ...
            <Input
              ...
              value={format(new IndianDate(), "PPP")}
              size={"md"}
            />
            <Input
              size="lg"
